...
public class QueryBuilder {
  ...
  public static String search() {
      ...
      sb.append(
        "isnull((select total from [dbo].[udfGetBalanceBySource](employeeid,fundid,"
      );
      sb.append(
        "(select max(valuedate) from UnitPrice where FundID = fundid))),0) as Balance,"
      );
    ...
    public static String build() {
      ...
      sb.append(" when cast(ContributionTypeID as varchar) in (17,18,32) then 'OST01'  ");
      ...
      sb.append(
        " when cast(ContributionTypeID as varchar) in (31) then	'NST01'  "
      );
      sb.append(" when cast(ContributionTypeID as varchar) in (2,34) then	'LEG01' ");
      sb.append(
        " when cast(ContributionTypeID as varchar) in (73,15,19,20) then 'RFD01'  "
      );
      sb.append(
        " when (cast(ContributionTypeID as varchar) in (9,37) AND c.Fundid=9) then 'TFD01'  "
      );
      //        sb.append(
      //                " when (cast(ContributionTypeID as varchar) in (37) AND c.Fundid=9) then 'TFD01' "
      //        );
      sb.append(
        " when (cast(ContributionTypeID as varchar) in (9,37) AND c.Fundid=1) then 'TFD02'  "
      );
      //        sb.append(
      //                " when (cast(ContributionTypeID as varchar) in (37) AND c.Fundid=1) then 'TFD02'  "
      //        );
      sb.append(
        " when (cast(ContributionTypeID as varchar) in (9,37) AND c.Fundid=10) then 'TFD03'  "
      );
      //        sb.append(
      ...
      //new added begins
      sb.append(
        " when cast(ContributionTypeID as varchar) in (25,26,48,52,58,61,64,65) then 'CAP01' "
      );
      sb.append(
        " when cast(ContributionTypeID as varchar) in (27,28,35,38,40,42) then 'RTD01' "
      );
      sb.append(
        " when cast(ContributionTypeID as varchar) in (33) then 'INP01'  "
      );
      sb.append(
        " when cast(ContributionTypeID as varchar) in (16) then 'ARR02'  "
      );
      sb.append(
        " when cast(ContributionTypeID as varchar) in (71) then 'NR001'  "
      );
      sb.append(
        " when cast(ContributionTypeID as varchar) in (27,28,35,38,40,42,33) then 'RTD01'  "
      );
      sb.append(
        " when cast(ContributionTypeID as varchar) in (65) then 'CAP01' "
      );
      // newly added ends
      ...
      sb.append(
        "NET_CONTRIBUTIONS = ISNULL(GrossAmount,0.00)- AdminFee, '0023' [RELATED_PFA_CODE] "
      );
      sb.append(
      ...
      sb.append(
        " when cast(PaymentTypeID as varchar) in (12,25, 43,44,46,52,53,54,55,58,61,63,68,70,79) then 'REV01' "
      );
      ...
      sb.append(" when cast(PaymentTypeID as varchar) in (6) then 'ANT01' ");
      //newly added ends
